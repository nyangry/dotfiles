...
tmux send-keys -t monitor.1 'watch -n 3 "ps -eo 'pid, %cpu, %mem, start, etime, command' | grep -E \"(vim|nvim|ruby|rspec|test|rails|puma|pa_|my_|spring|guard|bootsnap|mysql|redis|sidekiq|delayed_job|yarn)\" | grep -v -e grep -e fsevent -e start_host -e .cache | sort -b -f -k 6r,6r -k1"' C-m
